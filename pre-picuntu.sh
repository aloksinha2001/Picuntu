...
if [ -f /usr/bin/wget ] && [ -f /sbin/parted ]; 
	then
		...
		apt-get -qq -y install wget > /tmp/2 
		if [ -f /usr/bin/wget ]; 
		then
				WGET="Y"; 
		else
...
if [ -f /usr/bin/dialog ] && [ -f /usr/bin/pv ] ; 
	then
		rm -f /tmp/ubuntu 
	else
		echo "Trying to get dialog for nice menus"
		apt-get -qq -y install dialog > /tmp/2 
		if [ -f /usr/bin/dialog ]; 
		then
				rm -f /tmp/ubuntu;
		else
...
#				touch /tmp/ubuntu;
				# Force user to quit, and install dialog, pv
				clear
				echo "dialog and/or pv not available, cannot continue. Please install these applications and then re-run pre-picuntu"
		fi
fi
...
#http://download.g8.net/picuntu/pre-picuntu.sh
#http://download.g8.net/picuntu/picuntu-linuxroot-0.9-RC2.1.tgz

HST=`hostname`
if [ "$HST" == "monster" ]; 
	then  
		PIC_URL="http://192.168.1.112/picuntu-linuxroot-0.9-RC2.2.tgz"
		SHA_URL="http://192.168.1.112/picuntu-linuxroot-0.9-RC2.2.tgz-sha"
		PIC_FIL="picuntu-linuxroot-0.9-RC2.2.tgz"
	else
		PIC_URL="http://download.g8.net/picuntu/picuntu-linuxroot-0.9-RC2.2.tgz"
		SHA_URL="http://download.g8.net/picuntu/picuntu-linuxroot-0.9-RC2.2.tgz-sha"
		PIC_FIL="picuntu-linuxroot-0.9-RC2.2.tgz"
	fi
...
#		WEB_SUM=`wget -q "$SHA_URL" -O $TMP_SCRATCH ; grep sha1 $TMP_SCRATCH | cut -d '>' -f2 | cut -d '<' -f1`
		WEB_SUM=`wget -q "$SHA_URL" -O $TMP_SCRATCH ; grep sha1 $TMP_SCRATCH`
...
function conf_interfaces {
		...
		if [ ! $MODU == "1" ]; 
			then
				MSG="## /etc/modules: kernel modules to load at boot time." 
				MSG="$MSG ##"
				MSG="$MSG ## This file contains the names of kernel modules that should be loaded"
				MSG="$MSG ## at boot time, one per line. Lines beginning with # are ignored."
				MSG="$MSG \n\n## Modified by Pre-Picuntu"
				echo "$MSG" >  "$TMOUNT"/etc/modules
...
#		awk -v pat="$IFACE" '$0 ~ pat {while(getline && $0 != ""){}}1' "$TMOUNT"/etc/network/interfaces > "$TMP_SCRATCH1"
      ...
      # Creating /etc/network/interfaces
		echo "# Created by Pre-picuntu" > $TMP_SCRATCH
		echo "auto lo" >> $TMP_SCRATCH
		echo "iface lo inet loopback" >> $TMP_SCRATCH
		echo "" >> $TMP_SCRATCH
		echo "" >> $TMP_SCRATCH
		...
		echo "" >> $TMP_SCRATCH
		echo "" >> $TMP_SCRATCH
		cat $TMP_SCRATCH > "$TMOUNT/etc/network/interfaces"
...
# YNMSG with the message in the box
# Returns If yes - returns 0;If no - returns1; If escaped - returns 255  
$DIALOG --backtitle "$BACK_T" --title "$1" --yes-label "$3" --no-label "$4" --clear --yesno "$2" 21 70
return $?
}
...
# If something went wrong... check with user here
if [ ! -f "$TMOUNT/etc/network/interfaces" ]; 
	then
		read -p "Something went wrong, copying failed. Press Enter to exit and retry"
		exit_to_system
fi
...
'--nochecksum')
	...
	;;	
'--help')
	read -p "Help mode selected. Help coming soon... press enter to proceed."
	;;
*)
	DO_TXT="NO"
	do_main
